...
public class FontInfo {
    ...
    public FontInfo(ModuleFontInfoProperties moduleFontInfoProperties) {
        fontInfoProperties = new Properties();

        Optional<Module> module = ModuleUtil.getModule(moduleFontInfoProperties.getModuleName());

        module.ifPresentOrElse((m) -> {
                    try {
                        this.fontInfoProperties.load(m.getResourceAsStream(moduleFontInfoProperties.getUrlFontInfoProperties()));
                    } catch (IOException e) {
                        System.out.printf("An error accurred while loading '%s' of module %s: %s",
                                moduleFontInfoProperties.getUrlFontInfoProperties(), moduleFontInfoProperties.getModuleName(), e.getMessage());
                    }
                },
                () -> System.out.printf("Module not found %s", moduleFontInfoProperties.getModuleName()));
    }
